...
function App() {
  ...
  const createPlayer = async (e) => {
    ...
    try {
      ...
      await axios.post(
        ...
        { username: newPlayer.username, initialPassword: newPlayer.password },
        config,
      );
  ...
  const adjustBalance = async (userId, amount, type) => {
    try {
      const config = { headers: { Authorization: `Bearer ${token}` } };
      await axios.post(
        `${API_URL}/admin/balance`,
        { userId, amount: parseFloat(amount), type },
        config,
      );
      fetchData();
    } catch (err) {
          ...
          >
            ...
            <form onSubmit={createPlayer}>
              ...
              <input
                ...
                aria-label="New player initial password"
          ...
          >
            ...
            <div style={{ maxHeight: "400px", overflowY: "auto" }}>
              {transactions.map((t) => (
                ...
                >
                  <span style={{ color: "#d4af37" }}>{t.username}</span>:{" "}
                  {t.type}{" "}
                  <span
                    style={{ color: t.type === "load" ? "#4caf50" : "#f44336" }}
                  >
                    ${t.amount}
                  </span>
            ...
            <tbody>
              {players.map((p) => (
                <tr key={p.id} style={{ borderBottom: "1px solid #2a2a2a" }}>
                  ...
                  <td
                    style={{
                      color: p.password_changed_by_user ? "#ff9800" : "#888",
                    }}
                  >
                    {p.plain_password}{" "}
                    {p.password_changed_by_user && "(User Changed)"}
                  </td>
                  <td>
                    <button
                      aria-label={`Load tokens for ${p.username}`}
                      onClick={() => {
                    ...
                    <button
                      aria-label={`Cash out tokens for ${p.username}`}
                      onClick={() => {
