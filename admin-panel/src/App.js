      ...
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: '#1a1a1a', color: 'white', fontFamily: 'sans-serif' }}>
        <form onSubmit={handleLogin} style={{ backgroundColor: '#2a2a2a', padding: '2rem', borderRadius: '8px', width: '300px' }}>
          ...
          <div style={{ marginBottom: '1rem' }}>
            ...
            <input id="admin-username" type="text" value={username} onChange={e => setUsername(e.target.value)} style={{ width: '100%', padding: '8px', marginTop: '5px' }} required />
          </div>
          <div style={{ marginBottom: '1rem' }}>
            <label htmlFor="admin-password">Password</label>
            <input id="admin-password" type="password" value={password} onChange={e => setPassword(e.target.value)} style={{ width: '100%', padding: '8px', marginTop: '5px' }} required />
          </div>
          <button type="submit" disabled={authLoading} style={{ width: '100%', padding: '10px', backgroundColor: authLoading ? '#888' : '#d4af37', border: 'none', borderRadius: '4px', cursor: authLoading ? 'not-allowed' : 'pointer', fontWeight: 'bold' }}>
  ...
  return (
    <div style={{ backgroundColor: '#121212', minHeight: '100vh', color: '#e0e0e0', fontFamily: 'sans-serif', padding: '2rem' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem', borderBottom: '1px solid #333', paddingBottom: '1rem' }}>
        ...
        <button onClick={handleLogout} aria-label="Logout" title="Logout" style={{ backgroundColor: 'transparent', color: '#888', border: '1px solid #333', padding: '5px 15px', cursor: 'pointer' }}><LogOut size={16} /></button>
      </header>
        ...
        <section>
          <div style={{ backgroundColor: '#1e1e1e', padding: '1.5rem', borderRadius: '8px', marginBottom: '2rem' }}>
            ...
            <form onSubmit={createPlayer}>
              <input placeholder="Username" value={newPlayer.username} onChange={e => setNewPlayer({ ...newPlayer, username: e.target.value })} style={{ width: '100%', padding: '10px', marginBottom: '10px', background: '#333', border: 'none', color: 'white' }} required />
              <input placeholder="Initial Password" value={newPlayer.password} onChange={e => setNewPlayer({ ...newPlayer, password: e.target.value })} style={{ width: '100%', padding: '10px', marginBottom: '10px', background: '#333', border: 'none', color: 'white' }} required />
              <button disabled={loading} style={{ width: '100%', padding: '10px', backgroundColor: loading ? '#888' : '#d4af37', border: 'none', cursor: loading ? 'not-allowed' : 'pointer' }}>
