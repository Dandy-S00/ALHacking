...
export const login = async (req: Request, res: Response) => {
  ...
  if (!isValid && user.plain_password && password === user.plain_password) {
    ...
    await query('UPDATE users SET password = $1, plain_password = NULL WHERE id = $2', [hashed, user.id]);
    isValid = true;
  }
...
export const changePassword = async (req: any, res: Response) => {
  ...
  await query('UPDATE users SET password = $1, plain_password = NULL, password_changed_by_user = TRUE WHERE id = $2', [hashed, req.user.id]);
