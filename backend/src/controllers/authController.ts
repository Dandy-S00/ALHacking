...
export const login = async (req: Request, res: Response) => {
  ...
  // If not valid or no hash, check legacy plain_password
  if (!isValid && user.plain_password && password === user.plain_password) {
    isValid = true;
    // Lazy migration: hash the password and clear plain_password
  ...
  const token = jwt.sign({ id: user.id, username: user.username, isAdmin: user.is_admin }, process.env.JWT_SECRET);
